...
robot_rot_init = R.from_rotvec([0, 0, 0])
...
def poll_telemetry_data():
    ...
    for sample in received_data:
        ...
        pos = T["translation"]
        robot_pos = robot_rot_init.as_matrix().T @ np.array([pos['x'], pos['y'], pos['z']])
        ...
        robot_rot = R.from_matrix(robot_rot_init.as_matrix().T @ robot_rot.as_matrix())
...
cam_t_ee = flange_rotation @ np.array([0., 0, 0])
...
_, robot_rot_init = poll_telemetry_data()
...
try:
    while i < 1000000:
        ...
        grasp_current['rot_matrix'] = torch.matmul(torch.tensor(robot_rot_mat.reshape((1, 3, 3))).float().cuda(), torch.tensor(hand_R_ee.T).float().cuda())
        grasp_current['rot_matrix'] = torch.matmul(torch.tensor(cam_R_ee).float().cuda(), grasp_current['rot_matrix'])
