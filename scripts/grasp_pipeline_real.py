...
x_init = robot.get_cartesian_pose()[:3, 3:4].flatten()
...
# print(R.from_matrix(np.average(grasp_dict['rot_matrix'].detach().cpu().numpy(), axis=0)).as_euler('xyz'))
...
while i < 100:
    ...
    rot = R.from_matrix(grasp_dict_rot)
    ...
    x = cart_pose[:3, 3:4].flatten() - x_init
    cart_pose_R = cart_pose[:3, :3]
    ...
    dr_axis_update = rot[p_best].as_rotvec()
