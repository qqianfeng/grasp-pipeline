...
hand_R_ee[1,1] = -1
hand_R_ee[2,0] = -1
hand_R_ee = flange_rotation @ hand_R_ee

cam_R_ee = np.zeros((3, 3))
cam_R_ee[0,1] = -1
cam_R_ee[1,0] = -1
cam_R_ee[2,2] = -1
cam_R_ee = flange_rotation @ cam_R_ee

cam_t_hand = flange_rotation @ np.array([0., 0, 0.1])
...
def poll_telemetry_data():
    ...
    for sample in received_data:
        ...
        robot_rot = R.from_matrix(flange_rotation @ robot_rot.as_matrix().T)
...
k_p = [0.2, 0.5]                    # Constant control parameters
k_i = [0.0, 0.0]                    # Integrative control parameters
k_d = [0.,0.]#[0.5, 0.5]                    # Derivative control parameters
v_des_max = [0.05, 0.1]              # Limit of linear and angular velocity
...
try:
    while i < 1000000:
        ...
        if int(os.environ['VISUALIZE']):
            ...
            #grasp_dict['transl'] = torch.tensor(grasp_dict['transl'].detach().cpu().numpy() @ cam_R_ee.T + cam_t_hand).float().cuda()
        ...
        grasp_pos = grasp_dict['transl'].detach().cpu().numpy() @ cam_R_ee.T + cam_t_hand
        center_translation = (center_transf[:, :3] @ cam_R_ee.T + cam_t_hand)[0]
        ...
        grasp_current = dict()
        if simulation:
            grasp_current['transl'] = torch.tensor((robot_pos.reshape((1, -1)) - cam_t_hand) @ cam_R_ee).float().cuda()
            ...
            grasp_current['transl'] = torch.tensor(-cam_t_hand.reshape((1, -1)) @ cam_R_ee).float().cuda()
