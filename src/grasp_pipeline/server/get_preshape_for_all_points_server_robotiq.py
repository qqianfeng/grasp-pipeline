...
class GetPreshapeForAllPoints():
    ...
    def sample_robotiq_joint_state(self, max_weight=1.):
        """ Sample a joint state for the robotiq
        """
        ...
        # ++++++++++++Default open joint config preshape for Robotiq+++++++++++++++++++++
        # [palm_finger_1_joint, 
        # palm_finger_2_joint, 
        # finger_1_joint_1, 
        # finger_2_joint_1, 
        ...
        joint_pos_np = np.array(
            [0.005003288981476771, 
            0.005024192814866898, 
            0.0020109643795926625, 
            0.0018286023703515779, 
            0.0009078357336740905, 
            2.1030756791003114e-08, 
            -1.0356724663825085e-08, 
            5.9507813676661266e-09, 
            7.660365728978854e-10, 
            -0.0011456038887965097, 
            9.81300915248029e-09, 
            -1.467818403653126e-08])
        robotiq_joint_state = JointState()
        robotiq_joint_state.position = list(joint_pos_np)
        return robotiq_joint_state
    ...
    def handle_get_preshape_for_all_points(self, req):
        ...
        for i, (point, normal) in enumerate(zip(self.object_points, self.object_normals)):
            ...
            # Finally also sample a robotiq joint state
            max_weight = -0.08 * face.get_shorter_side_size() + 1.16
            joint_state = self.sample_robotiq_joint_state(max_weight=max_weight)
