...
import rospy
import time
from robotiq_3f_srvs.srv import Move, GetPosition
...
class HithandGraspController():
    def __init__(self):
        ...
        self.delta_vector_robotiq = np.array([
            self.delta_pos_joint_1
        ])
    ...
    #     try:
    #         get_robotiq_pos = rospy.ServiceProxy('/robotiq_3f_gripper/get_pos', GetPosition)
    #         res = get_robotiq_pos()
    ...
    def handle_grasp_control_robotiq(self, req):
        # First subscribe to the hithand joint state topic, which will continuously update the joint position at 100Hz
        joint_states_sub = rospy.Subscriber(
        ...
        # Initialize the delta vector every time as this changes during execution
        # delta_pos = np.tile(self.delta_vector, 5)
        delta_pos = self.delta_vector_robotiq
        joint_pos_msg = JointState()
        desired_pos = self.curr_pos
